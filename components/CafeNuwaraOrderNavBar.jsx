...
export const CafeNuwraOrderNavBar = () => {
  ...
  const handleCategoryClick = (name) => {
    const path = `/cafenuwara/order/${name.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`;
    router.push(path);
    closeBeverages();
  ...
  return (
    <div className="px-4 sm:px-10 mt-6">
      <div className="relative w-full bg-gradient-to-r from-amber-50 via-white to-amber-50 rounded-2xl shadow-lg border border-amber-200 p-6 overflow-visible">

        {/* Background image */}
        <div className="absolute inset-0 z-0 pointer-events-none pb-10 lg:pb-0 sm:pb-10">
          <Image
            src={cafenuwaranav_2}
            alt="Cafe Nuwara Background"
            fill
          ...
          <div className="hidden lg:flex flex-wrap justify-center items-start gap-4 xl:gap-6">
            ...
            {menuCategories.map((category, index) => (
              ...
              >
                <div className="flex flex-col items-center space-y-1 p-3 rounded-lg hover:bg-amber-100 transition-colors duration-200">
                  ...
                  <span className="text-sm font-semibold text-gray-800 group-hover:text-amber-700 transition-colors duration-200">
                    {category.name}
                  </span>
                </div>
              </div>
            ...
            <div className="flex flex-col items-start relative">
              ...
              >
                ...
                <span className="text-sm font-semibold text-gray-800 hover:text-amber-700 transition-colors duration-200">
                  Beverages
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {(showAllCategories
                ...
                <div
                  key={index}
                  onClick={() => handleCategoryClick(category.name)}
                  className="flex items-center space-x-2 p-3 bg-white rounded-lg shadow-sm border border-amber-100 hover:shadow-md transition-shadow duration-200 cursor-pointer"
                >
